...
const initGLTFLoader = (scene, morphs, mixer) => {
    ...
    gltfloader.load('assets/glb/Horse.glb', (gltf) => {
        const mesh = gltf.scene.children[0];
        const clip = gltf.animations[0];

        // Cavalo
        initMotph(scene, morphs, mixer, mesh, clip, 400, 1, 100 - Math.random() * 1000, -105, -50, true);
        initMotph(scene, morphs, mixer, mesh, clip, 400, 1, 100 - Math.random() * 1000, -105, -100, true);
        initMotph(scene, morphs, mixer, mesh, clip, 400, 1, 100 - Math.random() * 1000, -105, -200, true);
